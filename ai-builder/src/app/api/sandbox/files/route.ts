...
import { createOrConnectSandbox, listDirectory, writeFile, readFile, deleteFile } from '@/lib/e2b';
...
export async function GET(req: Request) {
  try {
    ...
    const path = searchParams.get('path') || '/home/user/app';
    ...
    if (action === 'read') {
      ...
      const files = await listDirectory(sandbox, path);
