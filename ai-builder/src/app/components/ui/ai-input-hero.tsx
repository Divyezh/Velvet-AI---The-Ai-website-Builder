...
export type HeroWaveProps = {
  ...
  extendLeftPx?: number;
  children?: React.ReactNode; // slot for the page's own hero content
};
...
export function HeroWave({
  ...
  useEffect(() => {
    ...
    function createInstancedMaterial() {
      // RED color scheme — changed from blue (#1f3dbc) to red (#e8232a / #8b0000)
      const baseCol = new THREE.Color("#e8232a");   // vivid red (was blue)
      const emisCol = new THREE.Color("#7a0010");   // deep crimson emissive (was dark blue)
    ...
    function setupPointerTracking() {
      ...
      listeners.push(() => {
        ...
        el.removeEventListener("touchmove", activate as any);
    ...
    function onResize(newW: number, newH: number) {
      ...
      if (barCount !== currentBarCount) {
        ...
        aX.needsUpdate = true; aT.needsUpdate = true;
      }
    ...
    gsap.to(waveContainer.querySelector("canvas"), { opacity: 1, duration: 1.2, ease: "power2.out" });
    ...
    return () => {
      ...
      try { (grainPass as any)?.dispose?.(); } catch {}
      try { (waveBloomPass as any)?.dispose?.(); } catch {}
      try { (waveComposer as any)?.dispose?.(); } catch {}
      try { (waveRenderer as any)?.dispose?.(); } catch {}
      try {
  ...
  return (
    ...
    >
      ...
      />
      {/* Children (page content) layered on top */}
      {children && (
